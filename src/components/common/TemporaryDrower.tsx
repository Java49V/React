...
export const TemporaryDraw: React.FC<Props> = ({ routes }) => {
  ...
  return (
    <Box sx={{ marginTop: { xs: '13vh', sm: '10vh' } }}>
      <CssBaseline />
      <AppBar
        position="fixed"
        open={open1}
        sx={{ backgroundColor: 'lightblue', height: { xs: '7vh', sm: '13vh' } }}
      >
        <Toolbar>
          <IconButton
            color="inherit"
            ...
            edge="start"
            sx={{ mr: 2, mb: 1.3, ...(open1 && { display: 'none' }) }}
          >
            <MenuIcon />
          </IconButton>
          <Typography

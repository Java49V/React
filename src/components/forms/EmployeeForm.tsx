...
export const EmployeeForm: React.FC<Props> = ({ submitFn, employeeUpdate }) => {
  const { minBirthYear, minSalary, maxBirthYear, maxSalary, department } =
    employeeConfig;
  const [employee, setEmployee] = useState<Employee>(
    employeeUpdate ? employeeUpdate : initialEmployee
  );
  ...
  const styleFontSize = { fontSize: '0.8em' };

  return (
    <Grid
      container
      justifyContent="center"
      width={{ sx: '80vw', sm: '80vw', md: '40vw' }}
      height={{ sx: '90vh', sm: '80vh', md: '50vh' }}
      rowSpacing={3}
    >
        ...
        <Paper style={styleGrid} elevation={7}>
          <form className="form" onSubmit={onSubmitFn} onReset={onResetFn}>
            <Grid
              ...
              justifyContent="center"
              width={{ sx: '80vw', sm: '80vw', md: '40vw' }}
              height={{ sx: '90vh', sm: '60vh', md: '50vh' }}
              marginTop={{ sm: '5vh' }}
            >
              ...
              <Grid item xs={10} sm={5} md={5}>
                <FormControl
                  sx={{ fontSize: { sx: '0.9rem', sm: '0.8rem', md: '1em' } }}
                  key="department"
                  fullWidth
                  required
                >
              ...
              <Grid item xs={10} sm={5} md={8}>
                <TextField
                  ...
                  helperText={`enter salary: ${minSalary}-${maxSalary}`}
                  inputProps={{
                    min: `${minSalary}`,
                    max: `${maxSalary}`,
                  }}
            ...
            >
              <Grid
                ...
                sm={3}
                md={2}
                marginBottom={{ sx: '1vh', sm: '2vh', md: '3vh' }}
              >
                <Paper>
                  <Button
                    sx={{
                      fontSize: { sx: '0.7rem', sm: '0.7rem', md: '0.9rem' },
                    }}
                    type="submit"
                  >
                    Submit
